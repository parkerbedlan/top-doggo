    ...
    .route("/name-dog", patch(
        |State(state): State<AppState>, Extension(context): Extension<AppContext>, Form(form): Form<NameDogFormParams>| async move {
            let err = |form_error: &str| {
                Html(name_dog_form(form.dog_id, FormField {value: form.new_name.clone(), error: form_error.to_string()}).into_string())
            };

            if form.new_name == "Jeff" {
            ...
            let result = sqlx::query!("UPDATE dog SET (name, namer_id) = ($1, $2) WHERE id = $3 RETURNING name", form.new_name, context.user_id, form.dog_id)
                .fetch_one(&state.pool).await;
            if result.is_err() {
                return err("C'mon, something more original!");
            }
            Html(html!{div class="text-xl" {(result.unwrap().name.unwrap())}}.into_string())
        }
    ))
}
...
fn name_dog_form(dog_id: i64, new_name: FormField<String>) -> Markup {
    html! {
        form class="flex gap-1" hx-patch="/name-dog" hx-swap="outerHTML" {
            input type="hidden" name="dog_id" value=(dog_id) ;
            div class="flex flex-col" {
                input type="text" id={"new_name_"(dog_id)} name="new_name" placeholder="Name this dog!" class={ "input input-bordered" @if new_name.error != "" {" !border-error"} } value=(new_name.value) ;
                label for={"new_name_"(dog_id)} class="label-text-alt text-error" {(new_name.error)}
            }
